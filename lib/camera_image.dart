...
class CameraState {
  CameraState._fromPlatformData(Map<dynamic, dynamic> data)
      : state = map(data["lowLightWarningState"]);

  final LowLightWarningState state;

  static LowLightWarningState map(String value) {
    //print("map to LowLightWarningState: " + value);

    switch (value) {
      case "bad":
        return LowLightWarningState.bad;
      case "low":
        return LowLightWarningState.low;
      default:
        return LowLightWarningState.ok;
    }
  }
}

enum LowLightWarningState { ok, low, bad }
